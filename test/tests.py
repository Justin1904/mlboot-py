...
from mlboot import ConfidenceIntervalEstimator, PairedConfidenceIntervalEstimator
...
def test_percentile(preds1, labels):
    ...
    a, b, scores = percentile(
        preds1,
        labels,
        score_func,
        np.arange(len(preds1)),
        confidence_level,
        sample_size,
        num_bootstrap,
    )
...
if __name__ == "__main__":
    ...
    for _ in range(n_runs):
        test_percentile(preds1, labels)
    gap = time.time() - start
    print(
        f"Running percentile test on {n} data points on average takes {gap / n_runs} seconds"
    )

    # test paired percentile
    start = time.time()
    for _ in range(n_runs):
        test_paired_percentile(preds1, preds2, labels)
    gap = time.time() - start
    print(
        f"Running paired percentile test on {n} data points on average takes {gap / n_runs} seconds"
    )

    # test bca
    start = time.time()
    for _ in range(n_runs):
        test_bca(preds1, labels)
    gap = time.time() - start
    print(
        f"Running BCa test on {n} data points on average takes {gap / n_runs} seconds"
    )

    # test paired BCa
    start = time.time()
    for _ in range(n_runs):
        test_paired_bca(preds1, preds2, labels)
    gap = time.time() - start
    print(
        f"Running paired BCa test on {n} data points on average takes {gap / n_runs} seconds"
    )

    start = time.time()
    for _ in range(n_runs):
        test_cluster_percentile(preds1, labels)
    gap = time.time() - start
    print(
        f"Running paired cluster percentile test on {n} data points on average takes {gap / n_runs} seconds"
    )
